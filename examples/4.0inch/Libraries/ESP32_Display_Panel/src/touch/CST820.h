...
#include "base/esp_lcd_touch_cst820.h"
#include "ESP_PanelTouch.h"

/**
 * @brief CST820 touch device object class
 *
 * @note  This class is a derived class of `ESP_PanelTouch`, user can use it directly
 */
class ESP_PanelTouch_CST820 : public ESP_PanelTouch {
public:
    /**
     ...
     */
    ESP_PanelTouch_CST820(ESP_PanelBus *bus, uint16_t width, uint16_t height, int rst_io = -1, int int_io = -1);
     ...
     */
    ESP_PanelTouch_CST820(ESP_PanelBus *bus, const esp_lcd_touch_config_t &config);

    /**
     * @brief Destroy the LCD device
     *
     */
    ~ESP_PanelTouch_CST820() override;
